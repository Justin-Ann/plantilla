<?php
// New index.php (converted from index.html)
session_start();

// Check if user is logged in
if(!isset($_SESSION["loggedin"]) || $_SESSION["loggedin"] !== true) {
    header("location: login.php");
    exit;
}
?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Plantilla Management System</title>
    <link rel="stylesheet" href="css/styles.css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
</head>
<body>
    <div class="container">
        <div class="sidebar">
            <div class="logo">
                <img src="img/dost-pagasa-logo.png" alt="DOST-PAGASA Logo">
            </div>
            <ul class="nav-menu">
                <li><a href="#" class="active" data-page="dashboard">DASHBOARD</a></li>
                <li><a href="#" data-page="data-management">DATA MANAGEMENT</a></li>
                <li><a href="#" data-page="applicants">APPLICANTS RECORDS</a></li>
                <li class="logout"><a href="logout.php">LOGOUT</a></li>
            </ul>
        </div>
        
        <div class="main-content">
            <div id="dashboard" class="page active">
                <h2>Dashboard</h2>
                <div class="status-cards">
                    <div class="card" id="on-process">
                        <h3>On-process</h3>
                        <p class="count">0</p>
                    </div>
                    <div class="card" id="on-hold">
                        <h3>On-hold</h3>
                        <p class="count">0</p>
                    </div>
                    <div class="card" id="not-yet-filling">
                        <h3>Not yet for filling</h3>
                        <p class="count">0</p>
                    </div>
                </div>
            </div>
            
            <div id="data-management" class="page">
                <h2>Data Management</h2>
                <div class="tabs">
                    <div class="tab active" data-tab="raw-data">Raw Data</div>
                    <div class="tab" data-tab="clean-data">Clean Data</div>
                </div>
                
                <div id="raw-data-content" class="tab-content active">
                    <div class="upload-section">
                        <h3>Upload Raw Data</h3>
                        <form id="upload-form" enctype="multipart/form-data">
                            <input type="file" name="file" accept=".xlsx,.xls,.csv">
                            <button type="submit">Upload</button>
                        </form>
                    </div>
                    
                    <div class="data-table">
                        <table id="raw-data-table">
                            <thead>
                                <tr>
                                    <th>PLANTILLA NO.</th>
                                    <th>PLANTILLADIVISION</th>
                                    <th>PLANTILLA SECTIONDEFINITION</th>
                                    <th>EQUIVALENTDIVISION</th>
                                    <th>POSITION TITLE</th>
                                    <th>ITEM NUMBER</th>
                                    <th>SG</th>
                                    <th>DATEVACATED</th>
                                    <th>VACATED DUE TO</th>
                                    <th>VACATED BY</th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
                    </div>
                </div>
                
                <div id="clean-data-content" class="tab-content">
                    <div class="filter-section">
                        <select id="status-filter">
                            <option value="">All Status</option>
                            <option value="On-process">On-process</option>
                            <option value="On-hold">On-hold</option>
                            <option value="Not yet for filling">Not yet for filling</option>
                        </select>
                        <button id="export-btn">Export to Excel</button>
                    </div>
                    
                    <div class="data-table">
                        <table id="clean-data-table">
                            <thead>
                                <tr>
                                    <th>PLANTILLA NO.</th>
                                    <th>PLANTILLADIVISION</th>
                                    <th>POSITION TITLE</th>
                                    <th>SG</th>
                                    <th>REMARKS</th>
                                    <th>DATE PUBLISHED</th>
                                    <th>STATUS</th>
                                    <th>ACTIONS</th>
                                </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
                    </div>
                </div>
            </div>
            
            <div id="applicants" class="page">
                <h2>Applicants Records</h2>
                <div class="search-section">
                    <input type="text" id="applicant-search" placeholder="Search by name...">
                    <button id="add-applicant-btn">Add New Applicant</button>
                </div>
                
                <div class="data-table">
                    <table id="applicants-table">
                        <thead>
                            <tr>
                                <th>FULLNAME</th>
                                <th>SEX</th>
                                <th>POSITION TITLE</th>
                                <th>TECHCODE</th>
                                <th>DATE OF BIRTH</th>
                                <th>DATE LAST PROMOTION</th>
                                <th>DATE LAST INCREMENT</th>
                                <th>DATE OF LONGEVITY</th>
                                <th>APPOINTMENT STATUS</th>
                                <th>ACTIONS</th>
                            </tr>
                        </thead>
                        <tbody></tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
    
    <!-- Modal for editing data -->
    <div id="edit-modal" class="modal">
        <div class="modal-content">
            <span class="close">&times;</span>
            <h2 id="modal-title">Edit Record</h2>
            <form id="edit-form">
                <input type="hidden" id="edit-id">
                <div class="form-group">
                    <label for="edit-remarks">Remarks:</label>
                    <select id="edit-remarks">
                        <option value="On-process">On-process</option>
                        <option value="On-hold">On-hold</option>
                        <option value="Not yet for filling">Not yet for filling</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="edit-date-published">Date Published:</label>
                    <input type="date" id="edit-date-published">
                </div>
                <div class="form-group">
                    <label for="edit-status">Status:</label>
                    <select id="edit-status">
                        <option value="On-process">On-process</option>
                        <option value="On-hold">On-hold</option>
                        <option value="Not yet for filling">Not yet for filling</option>
                    </select>
                </div>
                <button type="submit">Save Changes</button>
            </form>
        </div>
    </div>
    
    <!-- Modal for adding applicant -->
    <div id="applicant-modal" class="modal">
        <div class="modal-content">
            <span class="close">&times;</span>
            <h2>Add New Applicant</h2>
            <form id="applicant-form">
                <div class="form-row">
                    <div class="form-group">
                        <label for="fullname">Full Name:</label>
                        <input type="text" id="fullname" required>
                    </div>
                    <div class="form-group">
                        <label for="sex">Sex:</label>
                        <select id="sex">
                            <option value="Male">Male</option>
                            <option value="Female">Female</option>
                        </select>
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="position-title">Position Title:</label>
                        <input type="text" id="position-title" required>
                    </div>
                    <div class="form-group">
                        <label for="techcode">Techcode:</label>
                        <input type="text" id="techcode">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="date-of-birth">Date of Birth:</label>
                        <input type="date" id="date-of-birth">
                    </div>
                    <div class="form-group">
                        <label for="date-last-promotion">Date Last Promotion:</label>
                        <input type="date" id="date-last-promotion">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="date-last-increment">Date Last Increment:</label>
                        <input type="date" id="date-last-increment">
                    </div>
                    <div class="form-group">
                        <label for="date-of-longevity">Date of Longevity:</label>
                        <input type="date" id="date-of-longevity">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label for="appointment-status">Appointment Status:</label>
                        <select id="appointment-status">
                            <option value="PERMANENT">PERMANENT</option>
                            <option value="TEMPORARY">TEMPORARY</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="plantilla-no">Plantilla No:</label>
                        <select id="plantilla-no"></select>
                    </div>
                </div>
                <button type="submit">Save Applicant</button>
            </form>
        </div>
    </div>
    
    <script src="js/script.js"></script>
</body>
</html>